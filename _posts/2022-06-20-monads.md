...
Here is a use of Scheme's find operator.

```
> (find number? '(1 #f 2 dog))
1
```

Find takes a predicate and a list, and returns the first element of
...
```
> (find boolean? '(one #f two dog))
#f
```

Whoa ho ho. What've we got /here/?

find returns the first element of the list for which that predicate
holds. #f is a boolean, so that's what's returned. Lo, the confusion
that can result.

```
> (find boolean? '(one two dog))
#f
```
...
write my own find, and pick a different thing." Heck, we'll choose
...
```
> (my-find (lambda (y) (eqv? 'this-obscure-symbol-means-failure-Qx29t7 y))
...
If I want to return #f out of my find-maybe (which we'll write in a
...
```
> (find-maybe (lambda (v) (equal v `(Nothing))) '(-1 #f (Nothing) 404))
(Just (Nothing))
```

Ah-HA! Lookie there. Unlike before with Scheme's find, now we can tell
...
So that's the gist of what maybe is. Let's real quick write find-maybe
and, 'cause find-maybe doesn't show off everything we're gonna wanna
...
return (Nothing) when we can't find the thing. Whelp, that'll do it.
...
We wrap the thing up in the monad. find-maybe is written to have
...
recursive call. Well, find-maybe spits back something of Mbeta, which
...
Voilà. Go forth and find to your heart's content. I promised another
